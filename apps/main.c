...
#if TM_BENCH == 1
	configASSERT(pdPASS == xTaskCreateRestricted( vInitTaskBench(), &xTaskBench ));
#else
	configASSERT(pdPASS == xTaskCreateRestricted( vInitTask1(), &xTaskA ));
	configASSERT(pdPASS == xTaskCreateRestricted( vInitTask2(), &xTaskB )); //optimization does not work, pxEnclave1Definition is not initialized on GDB run.
#endif

	/* Test shared memory */
	/* Need to update EIDs */
	//~ configASSERT(pdPASS == xTaskCreateRestricted( vInitTaskO(), &xTaskO ));
	//~ configASSERT(pdPASS == xTaskCreateRestricted( vInitTaskT(), &xTaskT ));
}
/*-----------------------------------------------------------*/
